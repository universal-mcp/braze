...
@pytest.fixture
def app_instance():
    """Provides a BrazeApp instance for tests."""
...
def test_universal_mcp_braze_app_initialization(app_instance):
    """
    Test that the BrazeApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "braze", "BrazeApp instance has unexpected name."


def test_universal_mcp_braze_tool_docstrings_format(app_instance):
    """
    Test that each tool method in BrazeApp has a well-formatted docstring,
...
def test_universal_mcp_braze_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in BrazeApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
